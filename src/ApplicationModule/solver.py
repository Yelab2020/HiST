...
from ConvNeXtV2.loss import coxph_loss
...
from ConvNeXtV2.dataset import CoxDataset, LabelDataset
from torchsurv.loss.cox import neg_partial_log_likelihood
from ConvNeXtV2.model import ConvNeXtV2_TCGA, ConvNeXtV2_ICB
from ConvNeXtV2.metrics import AverageMeter, calculate_metrics
...
#         model.train()
#         for batch in train_loader:
#             x, day = [t.to(self.device) for t in batch] # only one batch
#             optimizer.zero_grad()
#             out = model(x)
#             out = out.reshape(-1)
#             loss = criterion(out,day)
#             loss.backward()
#             optimizer.step()
#             avg_meters['loss'].update(loss.item(), x.size(0))
...
#         model.eval()
#         with torch.no_grad():
#             x, day = [t.to(self.device) for t in next(iter(val_loader))]
#             out = model(x)
#             out = out.reshape(-1)
#             val_loss = criterion(out,day)
#             avg_meters['loss'].update(val_loss.item(), x.size(0))
...
#     ])
#         lr = self.lr * (len(train_loader)+len(val_loader)) / 256
#         cox_model = ConvNeXtV2_ICB(in_chans=in_chans, num_classes=self.num_classes,
#                                drop_path_rate=self.drop_path_rate, depths=self.depths, dims=self.dims)
#         cox_model = cox_model.to(self.device)
#         n_parameters = sum(p.numel() for p in cox_model.parameters() if p.requires_grad)
#         print('number of params:', n_parameters)
...
#         # Train loop
#         for epoch in range(self.epochs):
#             train_metrics = self._train(train_loader, cox_model, optimizer, MSELoss)
#             val_metrics = self._validate(val_loader, cox_model, MSELoss)
...
#             # Save the model if it achieves the lowest val_loss
#             best_model_path = os.path.join(out_dir, 'best_model.pth')
#             if epoch == 0:
#                 torch.save(cox_model.state_dict(), best_model_path)
#                 best_metric = val_metrics['loss']
#             elif val_metrics['loss'] < best_metric:
#                 torch.save(cox_model.state_dict(), best_model_path)
#                 best_metric = val_metrics['loss']
#                 if self.verbose:
#                     print("=> saved best model")
...
#             if self.verbose:
#                 print(f"Epoch: {epoch+1:03}, Training loss: {train_metrics['loss']:0.4f}, Val loss: {val_metrics['loss']:0.4f}")
...
#         mask_matrix_list: list,
#         he_features: list,
#         method: str = 'all',
#         kfold_splits = 5,
#         batch_size = 5,
#         out_dir = './checkpoint_PFS/'
#     ):
#         seed_torch(self.seed)
#         current_datetime = datetime.now().strftime('%Y%m%d%H%M')
#         out_dir = os.path.join(out_dir, current_datetime)
...
#         for idx_train,idx_val in kfold.split(range(len(PFS_day))):
#             save_dir = os.path.join(out_dir, f'fold_{n}')
#             if not os.path.exists(save_dir):
#                 os.makedirs(save_dir,exist_ok=True)
...
#             dataloader_train = DataLoader(
#                 LabelDataset(features = train_features,
#                             labels= [PFS_day[i] for i in idx_train],
#                             method = method),
#                             batch_size=batch_size, shuffle=False
#             )
#             dataloader_val = DataLoader(
#                 LabelDataset(features = val_features,
#                             labels= [PFS_day[i] for i in idx_val],
#                             method = method),
#                             batch_size=batch_size, shuffle=False
#             )
